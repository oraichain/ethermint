...
func (svd Eip712SigVerificationDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	evmAddressFromSigner := common.BytesToAddress(signerAddrs[i].Bytes())
	...
	if !bytes.Equal(accAddressFromPubkey, signerFromEvmAddressSigner) {
		return ctx, sdkerrors.Wrapf(sdkerrors.ErrorInvalidSigner,
...
func (spkd SetPubKeyDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for i, pk := range pubkeys {
		...
		evmAddressFromSigner := common.BytesToAddress(signers[i].Bytes())
